...
layout: post
title: 우리들의 코드
date: 2017-12-15 09:36:00
tags: code
author: 윤지상
---

<p>저희 코드는 사실 굉장히 많은 시행착오를 겪었습니다. 다음 url은 저희가 그동안 해왔던 코드들을 모아놓은 깃허브 url 입니다. <a herf = "https://github.com/beargrllys/Opensource"> in Beargrllys Github</a> </p>

<p> 그리고 다음 코드는 저희가 최종적으로 짠 코드이며 이전의 코드와 많이 다를 수 있습니다. 왜냐하면 서버 채널을 구현하는 과정에서 이전 코드의 구조상 어려움이 있다고 판단했고 따라서 그 동안의 경험을 기반 삼아 새로운 코드를 구성하였습니다.</p>
...
def main():
    ...
    while True:
        ...
        if(ik==i+1):
            ik=0
        if(LCDsetter(changetime,Amode, Bmode,title,ik)):
            ik+=1
